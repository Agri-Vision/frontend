...
const ReusableTable: React.FC<TableProps> = ({ columns, data, recordsPerPage = 5, onRowClick }) => {
  ...
  // Extract percentage from the disease vulnerability string
  ...
  const getBoxColor = (percentage: number | null) => {
    if (percentage !== null) {
    ...
    return 'transparent'; // Default color if no percentage
  };
              ...
              >
                {columns.map((column, colIndex) => (
                  <TableCell key={colIndex}>
                    {column.key === 'diseaseVulnerability' ? (
                      <Box
                        sx={{
                          display: 'inline-block',
                          padding: '4px 8px',
                          backgroundColor: getBoxColor(extractPercentage(row[column.key])),
                          borderRadius: '8px',
                        }}
                      >
