...
const MapDashboard: React.FC = () => {
  ...
  // Manually set the PNG file path here
  const pngFilePath = '../src/assets/maps/Gonadika-Holiday-Bungalow-Converted.png'; // Replace this with the actual path to your PNG file

  useEffect(() => {
    const initMap = async () => {
      if (mapRef.current && pngFilePath) { // Check if the map and PNG path are available
        const map = new window.google.maps.Map(mapRef.current, {
          zoom: 20,
          center: {
            lat: (7.1947473 + 7.1942133) / 2,
        ...
        // Get the geocoordinates from the GeoTIFF
        const geoCoordinates = await loadGeoTiffMetadata();
        if (!geoCoordinates) return;

        const bounds = new window.google.maps.LatLngBounds(
          new window.google.maps.LatLng(geoCoordinates.lowerRight.lat, geoCoordinates.lowerRight.lng),
          new window.google.maps.LatLng(geoCoordinates.upperLeft.lat, geoCoordinates.upperLeft.lng)
        );

        // Use the manually provided PNG file path as an overlay on the map
        class CustomOverlay extends window.google.maps.OverlayView {
          bounds: any;
          image: string;
          div: HTMLDivElement | null = null;
          ...
          onAdd() {
            ...
            img.src = this.image; // Use the provided PNG file path
            img.style.width = "100%";
            img.style.height = "100%";
            img.style.position = "absolute";
            this.div.appendChild(img);
        ...
        // Pass the manually set PNG file path to the overlay constructor
        const overlay = new CustomOverlay(bounds, pngFilePath);
        overlay.setMap(map);
        overlayRef.current = overlay;
      }
    };

    const loadGeoTiffMetadata = async () => {
      try {
        const tiffUrl = "../src/assets/maps/Gonadika-Holiday-Bungalow-4-24-2024-orthophoto.tif";
        ...
        // Get ModelPixelScale and ModelTiepoint tags
        const fileDirectory = image.getFileDirectory();
        const modelPixelScale = fileDirectory.ModelPixelScale; // [pixelWidth, pixelHeight, pixelDepth]
        const modelTiepoint = fileDirectory.ModelTiepoint; // [i, j, k, x, y, z]
        
        if (!modelPixelScale || !modelTiepoint) {
          throw new Error("ModelPixelScale or ModelTiepoint tags are missing in GeoTIFF metadata.");
        }
        
        // Calculate the geocoordinates for the upper-left and lower-right corners
        ...
        const lowerRight = {
          lng: upperLeft.lng + pixelWidth * width,
          lat: upperLeft.lat - pixelHeight * height,
        };
    
        console.log("Upper Left Coordinates:", upperLeft);
        console.log("Lower Right Coordinates:", lowerRight);
    
        // Use these coordinates to overlay on the map later
  ...
  }, [pngFilePath]); // Re-run the effect when the PNG file path changes
